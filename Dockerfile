...
#NEW STUFF
COPY --from=rust /app/target/release/libwfc.so priv/native/libwfc.so
#/NEW
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/wfc_app ./
COPY symmetry.json /app/
